...
【色彩意图优先规则】当用户需求中明显表达了明暗/色彩倾向时，style 与 palette 必以用户意图为准，一律不得套用随机的深色/暗色调种子：
- 浅色倾向词（如 雪、雪白、白、亮、浅色、明亮、清新、洁净 等）→ palette/style 必须采用浅色/明亮设计，即便随机种子是「暗色极客终端/赛博霓虹/深色底+荧光」等深色方向也要覆盖为浅色；
- 深色倾向词（如 暗、黑、夜、深色、暗黑、赛博朋克 等）→ 才允许采用深色设计，并以种子为基础展开。
若用户同时给出更具体的色彩描述（如「雪白底、冰蓝点缀」），严格遵循该描述。`
    ),
    new HumanMessage(
      `用户需求：${userRequest}${profileSection}

【随机设计方向种子】
本次必须以此组合为基础展开细化（仅当用户需求中已明确指定某维度的方向时，才用用户的覆盖对应维度，其余维度必须遵循种子；其中「明暗/色彩」为用户可控维度——若用户需求含明确浅色或深色倾向，palette 与 style 必须覆盖为用户的色调，不得沿用种子的深色或浅色方向）：
...
- 同一页出现多个绑定同一批文章/最新文章的动态文章区块（例如既放了「最新文章」article-list，又放了「精选 featured-articles」或「归档/时间线」归档列表，渲染后都展示同一批文章）。每个页面应只有一个文章流区块；命中请在 layout/issues 给出「合并/删减重复文章区块」的建议并拉低 layout 与 visualPolish 分。
